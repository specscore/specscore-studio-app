  ...
  template: ` <div class="layout-topbar">
    <div class="layout-topbar-logo-container">
      ...
      <a class="layout-topbar-logo" routerLink="/">
        ...
        <span style="white-space: nowrap">SpecScore.Studio App</span>
      </a>
    </div>
          ...
          @if (showUser()) {
            ...
            >
              @if (displayUser()!.photoURL) {
                <p-avatar
                  [image]="displayUser()!.photoURL!"
                  shape="circle"
                  size="normal"
                />
              } @else {
                <p-avatar
                  [label]="displayUser()!.displayName.charAt(0).toUpperCase()"
                  shape="circle"
                  size="normal"
                />
              }
            ...
            <p-popover #userMenu>
              <div class="flex flex-col gap-4 w-72">
                <div class="flex items-center justify-between">
                  <span class="text-sm font-semibold text-muted-color">Appearance</span>
                  <button
                    type="button"
                    class="layout-topbar-action"
                    (click)="toggleDarkMode()"
            ...
            <p-button
              ...
              icon="pi pi-sign-in"
              [text]="true"
              (onClick)="signInMenu.toggle($event)"
            />
            <p-menu #signInMenu [model]="signInItems" [popup]="true" />
          }
        </div>
      </div>
    </div>
...
export class AppTopbar {
  ...
  signInItems: MenuItem[] = [
    { label: 'GitHub', icon: 'pi pi-github', command: () => this.signInWith('github') },
    { label: 'Google', icon: 'pi pi-google', command: () => this.signInWith('google') },
    { label: 'Microsoft', icon: 'pi pi-microsoft', command: () => this.signInWith('microsoft') },
    { label: 'Apple', icon: 'pi pi-apple', command: () => this.signInWith('apple') },
    { separator: true },
    { label: 'Email', icon: 'pi pi-envelope', command: () => this.router.navigateByUrl('/auth/login') },
  ];
